// Minimal, dependency-free Code128-B barcode → SVG string generator.
const PATTERNS = [
...
export function code128SVG(value: string, opts?: { height?: number; module?: number }): string {
  ...
  const rects: string[] = [];
  for (const d of seq) {
    const w = parseInt(d, 10) * mod;
    if (bar) rects.push(`<rect x="${x.toFixed(2)}" y="0" width="${w.toFixed(2)}" height="${height}" fill="#000"/>`);
    x += w;
    bar = !bar;
  }
  const width = x;
