...
export async function POST(req: Request) {
  ...
  if (op === "create") {
    ...
    run("INSERT INTO accounts (id, business_id, name, type, opening_balance, created_at) VALUES (?,?,?,?,?,?)",
      [id, bid, body.name, body.type ?? "cash", Number(body.opening_balance) || 0, nowIso()]);
  ...
  if (op === "update") {
    ...
    run("UPDATE accounts SET name=?, type=?, opening_balance=? WHERE id=? AND business_id=?",
      [body.name, body.type ?? "cash", Number(body.opening_balance) || 0, body.id, bid]);
